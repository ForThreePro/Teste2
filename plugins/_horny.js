...
let handler = async (m, { conn, participants }) => {
    ...
    // ===== HORNY ===== CON BACKGROUND
    if (m.message?.extendedTextMessage?.text?.includes('horny') || m.text?.includes('horny')) {
        let who = m.mentionedJid[0] || m.quoted?.sender || m.sender
        let pp = await getAvatar(who)
        let apiUrl = `https://api.stellarwa.xyz/generate/horny?avatar=${encodeURIComponent(pp)}&background=${encodeURIComponent(defaultBg)}&key=${key}` // <- YA TIENE BG
...
━━━━━━━━━━━`)
            ...
            if(!res.ok ||!contentType.includes('image')) throw new Error(await res.text())
